...
{
    public static function getTileset(string $name): array
    {
        $tileset = self::getBuiltinTileset($name);

        if ($tileset[0] instanceof Tile) {
            self::checkTilesetStructure($tileset, Tile::class);
            return $tileset;
        }
        elseif ($tileset[0] instanceof TileDefinition) {
            self::checkTilesetStructure($tileset, TileDefinition::class);
            $factory = new TilesFactory($tileset, true);
            return $factory->createTiles();
        }
        else {
    ...
    private static function checkTilesetStructure($tileset, $class) : void
    {
        foreach ($tileset as $element) {
            if (!($element instanceof $class)) {
                throw new Exception('invalid tileset structure');
            }
        }
    }

    private static function getBuiltinTileset(string $name) : array
    {
        switch ($name) {
            case 'demo':
                return [
            ...
            default:
                throw new Exception("$name - no such tileset");
                break;
        }
    }
}
